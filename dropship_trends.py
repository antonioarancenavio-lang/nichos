...
def score_one(kw_id, query, name, day_snapshot, alerts):
    try:
...
def main():
    ...
    for item in keywords:
        ...
        score_one(item["id"], item["query"], item["name"], day_snapshot, alerts)

    # 2) Candidatos nuevos descubiertos hoy
    for item in pending:
        n += 1
        print(f"[{n}/{total}] (candidato) Consultando '{item['query']}'...")
        stats = score_one(item["id"], item["query"], item["name"], day_snapshot, alerts)
        if stats is None:
            continue

        if stats["week_avg"] >= PROMOTION_THRESHOLD:
            keywords.append({"id": item["id"], "name": item["name"], "query": item["query"]})
